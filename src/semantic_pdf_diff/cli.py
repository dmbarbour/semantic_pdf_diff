...
from .extract import extract_pdf, tiles
...
def main(argv=None):
    ...
    try:
        ...
        if args.plan:
            ...
            for label,path in [('A',args.a),('B',args.b)]:
                with fitz.open(path) as doc:
                    result[label] = {'pages':len(doc),'visual_tasks':sum(1 + (len(list(tiles(p.rect,settings.tile_points))) if max(p.rect.width,p.rect.height)>settings.tile_points else 0) for p in doc) if settings.vision else 0}
        ...
        data.update(schema_version=1, created_at=datetime.now(timezone.utc).isoformat(),documents=documents,
            ...
            settings=settings.model_dump(), usage={'api_calls':client.calls,'cache_hits':client.cache_hits,**client.usage},
