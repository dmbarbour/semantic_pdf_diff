...
def write_report(data, output):
    ...
    coverage = ''.join('<tr><td>'+esc(r['document'])+'</td><td>'+str(r['page'])+'</td><td>'+esc(r['source'])+'</td><td>'+esc(r['status'])+'</td><td>'+esc('; '.join(r['issues']))+'</td></tr>' for r in data['coverage'])
...
@media(max-width:700px){.pair{grid-template-columns:1fr}body{padding:0 12px}}
    ...
    page += f"<p>{esc(data['documents']['A']['name'])} ↔ {esc(data['documents']['B']['name'])}</p><p>Mode: {esc(data['mode'])}</p>"
    ...
    page += '<p>Revision mode treats A as the earlier version and B as the later version. Proposal mode treats both symmetrically; neither is ranked.</p>'
    ...
    page += '<details class="coverage"><summary>Source coverage ledger</summary><table><thead><tr><th>PDF</th><th>Page</th><th>Source</th><th>Status</th><th>Issues</th></tr></thead><tbody>'+coverage+'</tbody></table></details>'
