...
1. ✅ *Done 2026-09-24 (`3560327`; see [review](../reviews/store-m1-schema-v2-2026-09-24.md)).* **Schema v2:** content, file, source and interpreter model; content-relative locators; derivation chains; claim context fields (topic, basis, uncertainty, context, role) in the schema, filled by the prompt work in [scheduling-and-triage](scheduling-and-triage-2026-09-23.md); migrate `compare` and `report` off A/B.
2. ✅ *Done 2026-09-24 (`e4d76ff`; see [review](../reviews/store-m2-sqlite-store-2026-09-24.md)).* **Store folder and SQLite:** WAL, single-writer lock, evidence and coverage written per task in their own transactions, the response cache in the database with **semantic keys** (shared with [test-models-and-record-replay](test-models-and-record-replay-2026-09-24.md)), interpreter binding with rejection, selective `--reset` and `--dry-run`. **Resume by cache replay:** a rerun fast-forwards through cached answers (tested by interrupting a run midway). The persistent task queue moves to scheduling-and-triage (decided 2026-09-24; see the [milestone 1 review](../reviews/store-m1-schema-v2-2026-09-24.md)).
3. ↪ *Split and moved to consumers (see the [milestone 2 review](../reviews/store-m2-sqlite-store-2026-09-24.md)): document properties with milestone 4, source provenance with milestones 5 and 7, reviewer decisions with their first consumer.* **Annotations core:** the annotation table and record format; source provenance at registration and in manifests; extracted provenance from PDFs (document properties, sections, comments as content with context); export and import of reviewer decisions; orphan re-attachment. Capture from reports comes with its first consumer (reviewer feedback in scheduling-and-triage); model-recognized disagreements come with `check`.
4. **PDF sections:** outline → page ranges (font-size headings deferred: every multi-page sample PDF has an outline); native document properties into file metadata; heading path in prompts. When a table continues across pages, pass the previous chunk's table header and the section heading path with the continuation (affordable with the large context). Real documents are sensitive and won't be shared (the tool runs inside a multi-layer sandbox), so heuristics are developed against the public corpus (see [Samples](#samples)) and must fall back gracefully when there's no outline or consistent heading font.
